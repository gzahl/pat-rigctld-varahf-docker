...
#!/bin/sh
WINEPREFIX=$WINE32PREFIX WINEARCH=win32 box86 ~/wine/bin/wine \$@
EOF
cat << EOF > /usr/local/bin/wine64
#!/bin/sh
WINEPREFIX=$WINE64PREFIX WINEARCH=win64 box64 ~/wine/bin/wine64 \$@
EOF
cat << EOF > /usr/local/bin/wineserver
#!/bin/sh
WINEPREFIX=$WINE64PREFIX WINEARCH=win64 box64 ~/wine/bin/wineserver \$@
EOF
