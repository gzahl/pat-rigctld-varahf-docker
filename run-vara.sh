...
DISPLAY=:0
export DISPLAY
WINEPREFIX=$WINE32PREFIX
export WINEPREFIX

if [ ! -f $WINEPREFIX/drive_c/VARA/VARA.exe ]; then
	...
	mv 'VARA setup (Run as Administrator).exe' $WINEPREFIX/drive_c/vara_setup.exe
	wine $WINEPREFIX/drive_c/vara_setup.exe /SILENT
fi
